...
It's not possible to send logs via the OpenTelemetry collector to Splunk, 
as the [OpenTelemetry Collector AWS Lambda Extension layer](https://github.com/open-telemetry/opentelemetry-lambda/tree/main/collector) 
doesn't include the Splunk HEC exporter.  However, we can ingest the logs into 
Splunk platform from AWS CloudWatch, and make them available in 
Splunk Observability Cloud using Log Observer Connect. This ensures we have 
full correlation between traces generated by AWS Lambda instrumentation with metrics and logs.

The Splunk Distribution of OpenTelemetry Python includes the ability 
to automatically inject trace context into log entries, such as in the following example: 
